...
def prep_display(dets_out, img, h, w, undo_transform=True, class_color=False, mask_alpha=0.45, fps_str=''):
    ...
    if segmentation==1:
        ...
        if args.display_masks and cfg.eval_mask_branch and num_dets_to_consider > 0:
            # After this, mask is of size [num_dets, h, w, 1]
            masks = masks[:num_dets_to_consider, :, :, None]
            img_gpu2 =img_gpu * (masks.sum(dim=0) >= 1).float().expand(-1, -1, 3)
            img_gpu-=img_gpu2
        ...
        if mode==0:
            ...
            img_numpy2= cv2.cvtColor(img_numpy2, cv2.COLOR_BGR2GRAY)
            img_numpy2=cv2.cvtColor(img_numpy2,cv2.COLOR_GRAY2RGB)
            img_numpy += img_numpy2
        elif mode==1:
            img_numpy = (img_gpu * 255).byte().cpu().numpy()
            img_numpy2 = (img_gpu2 * 255).byte().cpu().numpy()
            img_numpy= cv2.cvtColor(img_numpy, cv2.COLOR_BGR2GRAY)
            img_numpy=cv2.cvtColor(img_numpy,cv2.COLOR_GRAY2RGB)
            img_numpy += img_numpy2
        elif mode==2:
            img_numpy = (img_gpu * 255).byte().cpu().numpy()
            img_numpy2 = (img_gpu2 * 255).byte().cpu().numpy()
            black_pixels = (img_numpy2 == (0, 0, 0)).all(axis=-1)
            img_numpy2[black_pixels]=(255,255,255)
            h, w, _ = img_numpy2.shape
            for i in range(h):
                for j in range(w):
                    m=min(img_numpy2[i,j])
                    #print(m)
                    img_numpy[i,j]=[m,m,m]
        elif mode==3:
            s=0.5
            img_numpy = (img_gpu * 255).byte().cpu().numpy()
            img_numpy2 = (img_gpu2 * 255).byte().cpu().numpy()
            img_numpy3= cv2.cvtColor(img_numpy2, cv2.COLOR_BGR2GRAY)
            #print(img_gpu.shape)
            h, w, _ = img_numpy2.shape
            for i in range(h):
                for j in range(w):
                    img_numpy[i,j]=[(1+s)*img_numpy2[i,j,0]-s*img_numpy3[i,j],(1+s)*img_numpy2[i,j,2]-s*img_numpy3[i,j],(1+s)*img_numpy2[i,j,2]-s*img_numpy3[i,j]]
        elif mode==4:
            img_numpy = (img_gpu * 255).byte().cpu().numpy()
            img_numpy2 = (img_gpu2 * 255).byte().cpu().numpy()
            img_hsv = cv2.cvtColor(img_numpy2,cv2.COLOR_BGR2HSV)  # 色空間をBGRからHSVに変換
            ...
            img_numpy2 = cv2.cvtColor(img_hsv,cv2.COLOR_HSV2BGR)  # 色空間をHSVからBGRに変換
            img_numpy= cv2.cvtColor(img_numpy, cv2.COLOR_BGR2GRAY)
            img_numpy=cv2.cvtColor(img_numpy,cv2.COLOR_GRAY2RGB)
            img_numpy += img_numpy2
        elif mode==5:
            ...
            v_magnification = 1.2  # 明度(Value)の倍率
